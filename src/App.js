...
export default function App() {
  ...
  useEffect(() => {
    document.onselectionchange = function () {
      ...
      console.log('from', anchorNode?.getAttribute('data-id'));
      console.log('to', focusNode?.getAttribute('data-id'));
    };
  });
  return (
    <Container>
      ...
      <Wrapper gap={gap} rows={rows} onSelect>
        {Array.from(Array(Number(box)).keys()).map((item, key) => (
          <Grid data-id={item} key={key}></Grid>
        ))}
      </Wrapper>
    </Container>
  );
